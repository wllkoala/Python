  ...
  """
    ...
    if encrypted_filename is None:
        ...
        encrypted_filename = "".join(
            filename.split('.')[:-1]) + '_' + 'encrypted' + '.pdf'
  ...
  """
    ...
    if decrypted_filename is None:
        decrypted_filename = "".join(
            filename.split('.')[:-1]) + '_' + 'decrypted' + '.pdf'
    # 写入新文件
...
def split_by_pages(filename, pages, password=None):
  ...
  """
    ...
    if pages <= 1:
        ...
        return
    # 得到切分之后每个pdf文件的页数
    pdf_num = pages_nums // pages + 1 if pages_nums % pages else int(
        pages_nums / pages)
    ...
    for cur_pdf_num in range(1, pdf_num + 1):
        ...
        pdf_writer = PdfFileWriter()
        # 生成对应的文件名称
        split_pdf_name = "".join(filename)[:-1] + '_' + str(
            cur_pdf_num) + '.pdf'
        # 计算出当前开始的位置
